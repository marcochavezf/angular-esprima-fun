...
function createProjectSemantics(dir, callback, enableVerbose){
  ...
  walker.on('end', function() {
    ...
    var filtersSemantics = _.reduce(filesParsed, function(accum, fileParsed){
      var filters = _.map(fileParsed.fileSemantic.filters, function(filter){
        filter.pathFile = fileParsed.pathFile;
        return filter;
      });
      return accum.concat(filters)
    }, []);
