...
module.exports = {
  ...
  getNodeFunctionFromBodyAndGlobals: getNodeFunctionFromBodyAndGlobals,
...
function getFunctionByIdentifier(bodyToAnalize, nameIdentifier){
  var functionData = {
    functionMeta: null,
    function: null
  };
...
function getNodeFunctionFromBodyAndGlobals(nodeIdentifier, bodyToAnalize, globalFunctions){
  switch (nodeIdentifier.type) {
    case 'FunctionExpression':
      return nodeIdentifier;

    case 'Identifier':
      var functionData = getFunctionByIdentifier(bodyToAnalize, nodeIdentifier.name);

      //if function node is not contained in body, then look for function in global functions.
      if (_.isNil(functionData.function)){
        return getBodyFunction({ node: nodeIdentifier }, globalFunctions);
      }
