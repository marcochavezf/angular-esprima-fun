  ...
  for (var i=0; i<directives.length; i++){
    ...
    if (directiveController) {
      ...
      appendPropAndFunctions({
        ...
        functionWithScopeMeta: directiveMeta.controller,
        functionWithScope: directiveController.value });
    }
...
function appendPropAndFunctions({ directiveMeta, functionWithScopeMeta, functionWithScope, globalFunctions, useFirstParamAsScopeId }){
  switch (functionWithScope.type) {
    case 'FunctionExpression':
      var scopeNameIdentifier = _.head(functionWithScope.params).name;
      functionWithScopeMeta.function = functionWithScope;
    ...
    case 'Identifier':
      estraverse.traverse(directiveMeta.function.body, {
        enter: fileHelper.getBodyByIdentifier(functionWithScope.name, function(controllerFunction){
          if (controllerFunction.init) {
            functionWithScopeMeta.functionMeta = controllerFunction;
            functionWithScopeMeta.function = controllerFunction.init;
          } else {
            functionWithScopeMeta.function = controllerFunction;
          }
        })
      });

      //Look for function in global functions and append structure, other wise just get scope properties and functions.
      if (_.isUndefined(functionWithScopeMeta.function)){
        functionWithScopeMeta.node = functionWithScope;
        ctrlrHelper.appendControllersStructure([functionWithScopeMeta], globalFunctions);
      } else {
        var scopeNameIdentifier = '$scope';
        if (useFirstParamAsScopeId){
          scopeNameIdentifier = _.head(directiveMeta.link.function.params).name;
        }
        ctrlrHelper.appendControllerPropAndFunctions(functionWithScopeMeta, scopeNameIdentifier);
      }
      break;
