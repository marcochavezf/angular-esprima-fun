...
},{"./_root":336}],330:[function(require,module,exports){
...
},{"./_Symbol":329,"./_getRawTag":333,"./_objectToString":334}],331:[function(require,module,exports){
(function (global){
...
},{"./_overArg":335}],333:[function(require,module,exports){
var Symbol = require('./_Symbol');
...
},{"./_Symbol":329}],334:[function(require,module,exports){
/** Used for built-in method references. */
var objectProto = Object.prototype;
...
},{"./_freeGlobal":331}],337:[function(require,module,exports){
/**
...
},{"./_baseGetTag":330,"./_getPrototype":332,"./isObjectLike":337}],339:[function(require,module,exports){
...
},{"./compose":343}],341:[function(require,module,exports){
'use strict';
...
},{"./createStore":344,"./utils/warning":346,"_process":339,"lodash/isPlainObject":338}],343:[function(require,module,exports){
"use strict";
...
},{"lodash/isPlainObject":338,"symbol-observable":347}],345:[function(require,module,exports){
(function (process){
'use strict';
...
},{"./applyMiddleware":340,"./bindActionCreators":341,"./combineReducers":342,"./compose":343,"./createStore":344,"./utils/warning":346,"_process":339}],346:[function(require,module,exports){
'use strict';
...
var _boundActions = require('./bound-actions');

var boundActions = _interopRequireWildcard(_boundActions);
...
//'starter.controllers',
'arvak.forms', 'angular-cache', 'arvak.common', 'arvak.auth', 'arvak.env', 'arvak.version', 'arvak.navigation', 'arvak.tasks', 'arvak.location', 'arvak.audio', 'arvak.update', 'arvak.appdef', 'checklist-model', 'ui.slider', 'angularFileUpload', 'templates', 'angular-locker', 'btford.socket-io', 'ngMockE2E', 'ngMessages', 'angular-cache', 'ngWebworker', 'ionic.cloud', 'clientsideCommons', 'ngRedux']).config(AppConfiguration.Logger).config(function ($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider, $ionicConfigProvider, $ionicCloudProvider, $compileProvider, CacheFactoryProvider, $ngReduxProvider, $provide, EnvironmentConfig) {
	$ionicCloudProvider.init({
		core: { app_id: '00cba80b' }
	});
	//Initialize REDUX store
	$ngReduxProvider.createStoreWith(_reducers2.default);
...
}).run(function ($ngRedux, $animate, $ionicPlatform, $rootScope, $ionicDeploy, WebUpdateFactory, MobileConfig, EnvironmentConfig, $templateCache, $httpBackend, paths, OnEventListener) {
	...
	$ionicPlatform.ready(function () {
		...
		// check updates from ionic deploy
		if (MobileConfig.isDevice() && !EnvironmentConfig.isLiveDeployDisabled) {
			WebUpdateFactory.check(EnvironmentConfig.env).then(function (hasUpdate) {
				if (hasUpdate) {
	...
	getStatusLabelById: function getStatusLabelById(id) {
		var label = null;
	...
	ddTypes: {
		...
		geolocation: 10,
		user_assignment: 37,
		relationship_entity_parent: 40,
		relationship_org_parent: 31,
		role_lookup: 32,
		task_status: 44,
		validate: 54
	},
...
function setMockResponses($httpBackend, isMockBackendEnabled) {
   ...
   */
		...
		$httpBackend.whenGET(paths.host() + paths.app.definitions()).respond(getMockResponseData('api.mobile.def.apps.json'));

		$httpBackend.whenGET(paths.host() + paths.user.current()).respond(getMockResponseData('api.user.json'));

		$httpBackend.whenGET(paths.host() + paths.datasources.getAllDSTypes()).respond(getMockResponseData('api.datasources.types.json'));

		$httpBackend.whenGET(paths.host() + paths.media.bucket()).respond('{"status":"ok","response":"com.primotus.mediatest"}');

		$httpBackend.whenGET(paths.host() + paths.workflow.getCurrentTasks(null)).respond(getMockResponseData('api.workflows.workorderdefinition.userTasks.json'));

		$httpBackend.whenGET(paths.host() + paths.workorders.getProcessForms()).respond(getMockResponseData('api.workflows.workorderdefinition.v2.processForms.json'));

		$httpBackend.whenGET(/.*\/api\/workflows\/workorderdefinition\/userTasks?.*/g).respond(getMockResponseData('api.workflows.workorderdefinition.userTasks.json'));

		$httpBackend.whenGET(/.*\/api\/workflows\/workorderdefinition\/userTasks\?tasksAfter=.*/g).respond(getMockResponseData('api.workflows.workorderdefinition.userTasks.json'));

		$httpBackend.whenGET(/.*\/api\/workflows\/workorderdefinition\/v2\/processForms?.*/g).respond(getMockResponseData('api.workflows.workorderdefinition.v2.processForms.json'));

		$httpBackend.whenGET(/.*\/api\/workflows\/workorderdefinition\/v2\/processForms\?procDefIds=.*/g).respond(getMockResponseData('api.workflows.workorderdefinition.v2.processForms.json'));
...
angular.module('arvak').factory('HttpInterceptor', HttpInterceptor);
...
},{"./bound-actions":351,"./redux/reducers":355,"babel-polyfill":1}],351:[function(require,module,exports){
'use strict';
...
var reduxActions = _interopRequireWildcard(_actions);

var _taskHelperService = require('./taskHelperService');

var taskHelper = _interopRequireWildcard(_taskHelperService);

var _webWorkerTasks = require('./webWorkerTasks');

var webWorkerTasks = _interopRequireWildcard(_webWorkerTasks);
...
/* @ngInject */
function TaskActions($http, $interval, $rootScope, paths, UserService, $q, $ngRedux, PouchService, NavigationService, ArvakConfig, TaskService, FormService, ConnectionService, $timeout, Webworker, ErrorService) {

	var service = {
		buildMenu: buildMenu,
		buildNextView: buildNextView,
		buildTableForDefaultTasks: buildTableForDefaultTasks,
	...
	var processUnsentTasksPromise = null;
	var latestAfterSubmitPromise = null;
	var tasksFromServerPromise = null;
	...
	/**
  * This function will enable/disable spinner loader, but we'll only want to disable the spinner when redux
  * has finished to process data (TaskActions.subscribeUserTasks in menuBuilderController), because otherwise
  * there will be an gap of one second or more between updating the tasks and hiding the spinner.
  * @param isDataLoading
  */
	function sendDataLoadingFlag(isDataLoading) {
		$rootScope.$broadcast('dataLoading', isDataLoading);
	}

	function stop() {
		unsubscribeUserTasks();
		cancelAutoPolling();
		latestAfterSubmitPromise = null;
		processUnsentTasksPromise = null;
		_dataLoading = null;
		sendDataLoadingFlag(_dataLoading);
	}

	function init() {

		function persist(state) {
			//Check if redux state is completed (sometimes a property as 'tasks' could take a while because tasks are retrieved from pouchdb)
			if (_.isEmpty(state.tasks) || _.isEmpty(state.views)) {
				return;
			}
			//We do a quick comparison with the previous redux state to save the new state
			if (previousStateStored && _.isEqual(previousStateStored.views, state.views) && _.isEqual(previousStateStored.tasks.list, state.tasks.list)) {
				return;
			}
			...
			//Save redux state without task instances data
			var statePromise = PouchService.putDoc({
				_id: stateKey,
				state: { views: state.views, tasks: { list: state.tasks.list } }
			}, PouchService.reduxState);
			var promises = [statePromise];

			//Save task data that have changed in the redux state
			if (state.tasks.instances) {
				_.each(state.tasks.instances, function (task, taskId) {
					if (!_.isNil(previousStateStored) && _.isEqual(task, previousStateStored.tasks.instances[taskId])) {
						return;
					}
					var id = stateKey + ':' + taskId;
					var taskPromise = PouchService.putDoc({ task: task, _id: id }, PouchService.taskDB);
					promises.push(taskPromise);
				});
			}

			//Remove tasks that are not present in the current redux state
			if (!_.isNil(previousStateStored)) {
				_.each(previousStateStored.tasks.list, function (prevTaskId) {
					var isContainedInCurrentState = _.find(state.tasks.list, function (currentTaskId) {
						return currentTaskId === prevTaskId;
					});
					if (!isContainedInCurrentState) {
						//we add angular.noop to resolve $q.all successfully, we just remove trash data if it exists
						var id = stateKey + ':' + prevTaskId;
						var removeTaskPromise = PouchService.deleteDoc(id, PouchService.taskDB).catch(angular.noop);
						promises.push(removeTaskPromise);
					}
				});
			}
		...
		var debouncePersist = _.debounce(persist, 5000, { 'trailing': true });
	...
	function loadStoredReduxState() {
		...
		var appDefId = NavigationService.getAppState().id;
		var stateKey = userId + ':' + appDefId;
		return PouchService.getDoc(stateKey, PouchService.reduxState).then(function (doc) {
			var state = doc.state;
			if (!_.has(state, 'tasks')) {
				return;
			}

			//Load Tasks and views only if there were tasks stored by userId
			var promises = _.map(state.tasks.list, function (taskId) {
				var id = stateKey + ':' + taskId;
				return PouchService.getDoc(id, PouchService.taskDB);
			});
	...
	function buildNextView(view, parentFilters, tasks) {
		...
		var userId = $rootScope.user.id.fieldValue;
		var appDefId = NavigationService.getAppState().id;
		var filters = [].concat(_toConsumableArray(parentFilters));
	...
	function processUnsentTasks() {
		if (!processUnsentTasksPromise) {
			processUnsentTasksPromise = getProcessUnsentTasksPromise();
		}
		return processUnsentTasksPromise;
	}

	function getProcessUnsentTasksPromise() {
		if (!ConnectionService.isOnline()) {
			_dataLoading = false;
			sendDataLoadingFlag(_dataLoading);
			return $q.when(false).finally(function () {
				processUnsentTasksPromise = null;
			});
		}
		...
		return TaskService.getProcessForms(processIds).then(function (response) {
			...
			return getUserTasks().then(function (userTasks) {
				...
				_.each(completeTasks, function (completeTask) {
					var taskToSubmit = Object.assign({}, completeTask.taskToSubmit);
					if (_.isUndefined(taskToSubmit.form)) {
						taskToSubmit.form = taskHelper.getTaskFormById(taskToSubmit.formDefinitionId, taskForms);
					}
				...
				return $q.all(unsentPromises).then(function (response) {
					...
					_.each(response, function (response) {
						var task = response.instance;
						var originalInstance = _.find(completeTasks, { id: task.taskdata.dataSourceInstanceId });
						if (response.status === 200) {
							//onSuccess, remove task
							removeTask(originalInstance).catch(function (error) {
								throw error;
							});
						} else {
							//onError:  change status to error and update task
							task.originalInstance = originalInstance;
							handleErroredTask({ task: task, error: response });
						}
					});

					_dataLoading = false;
					//we'll disable the spinner loader in TaskActions.subscribeUserTasks (menuBuilderController)
					//sendDataLoadingFlag(_dataLoading);
					return response;
				}).catch(function (errorResponse) {
					var task = errorResponse.task;
					task.originalInstance = _.find(completeTasks, { id: task.taskdata.dataSourceInstanceId });
			...
			console.log("processUnsent promise deregistered");
			processUnsentTasksPromise = null;
		});
	}
    ...
    * We don't consider 502 because it could be breaking something in backend and we want to avoid replicating 502 requests. */
			switch (errorResponse.error.status) {
				case 504:
				case -1:
					//TODO: check 'status' property from new contract
					instance.taskStatus = ArvakConfig.taskStatus.complete;
					break;

				default:
					//TODO: check 'status' property from new contract
					instance.taskStatus = ArvakConfig.taskStatus.error;
			}
			return updateTasks([instance]).catch(function (error) {
				//console.log(error);
	...
	function getTasksFromServer(userTaksSettings) {
		console.log("Getting tasks Fro Server");
		if (_.isEmpty(userTaksSettings) || _.isUndefined(userTaksSettings)) {
			userTaksSettings = { lastSyncTime: null, removedTaskInstIds: [] };
		}
		var lastSyncTime = userTaksSettings.lastSyncTime;
		var removedTaskInstIds = userTaksSettings.removedTaskInstIds;
		var processIds = NavigationService.getAppState().properties.processIds;

		var procDefIds = _.isArray(processIds) ? processIds.join(',') : '';
		var taskInstIds = _.isArray(removedTaskInstIds) ? removedTaskInstIds.join(',') : '';
		var payload = {};
		if (!_.isEmpty(procDefIds)) {
			payload.procDefIds = procDefIds;
		}
		if (!_.isEmpty(taskInstIds)) {
			payload.taskInstIds = taskInstIds;
		}
		if (!_.isNil(lastSyncTime) && _.isEmpty(taskInstIds)) {
			payload.tasksAfter = lastSyncTime;
		}
		var endpoint = paths.host() + paths.workflow.getCurrentTasksV2();
		return $http.post(endpoint, payload, { timeout: 300000 }).then(function (response) {
			var tasks = response.data.response.instances;
			//var tasksCount = response.data.response.tasksCount;
			...
			return UserService.getUserSettings().then(function (userSettings) {
				...
				var removedTaskUpdated = false;
				userSettings.general.removedTaskInstIds = _.reject(userSettings.general.removedTaskInstIds, function (taskInstId) {
					removedTaskUpdated = _.find(tasks, function (task) {
						return taskInstId === task.id;
					});
					return removedTaskUpdated;
				});

				userSettings.general.lastSyncTime = milliseconds;
				if (removedTaskUpdated) {
					userSettings.general.openTasksIds = _.union(userSettings.general.openTasksIds, response.data.response.openTasksIds);
				} else {
					userSettings.general.openTasksIds = response.data.response.openTasksIds;
				}
				//userSettings.general.tasksCount = tasksCount;
	...
	function transformAllTasks(tasks) {
		return webWorkerTasks.createWebWorker({
			Webworker: Webworker,
			webWorkerFunction: webWorkerTasks.transFormTasksWorker,
			data: { tasks: tasks, ArvakConfig: ArvakConfig }
		});
	}
	...
	function formatUserTasksReduxModel(tasks) {
		return webWorkerTasks.createWebWorker({
			Webworker: Webworker,
			webWorkerFunction: webWorkerTasks.formatUserTasksReduxModelWorker,
			data: { tasks: tasks, ArvakConfig: ArvakConfig }
		});
	}

	function buildTableForDefaultTasks(_ref) {
		var tasks = _ref.tasks,
		    taskFilter = _ref.taskFilter,
		    processFields = _ref.processFields,
		    fieldSettings = _ref.fieldSettings,
		    nextViewId = _ref.nextViewId;

		return webWorkerTasks.createWebWorker({
			Webworker: Webworker,
			webWorkerFunction: webWorkerTasks.buildForNormalTasksWebWorker,
			data: { tasks: tasks, taskFilter: taskFilter, processFields: processFields, fieldSettings: fieldSettings, nextViewId: nextViewId, ArvakConfig: ArvakConfig }
		});
	}

	function sortRawTasks(_ref2) {
		var tasks = _ref2.tasks,
		    sortDirection = _ref2.sortDirection,
		    fieldIndex = _ref2.fieldIndex,
		    taskFilter = _ref2.taskFilter,
		    processFields = _ref2.processFields;

		return webWorkerTasks.createWebWorker({
			Webworker: Webworker,
			webWorkerFunction: webWorkerTasks.sortRawTasksWebWorker,
			data: { tasks: tasks, sortDirection: sortDirection, fieldIndex: fieldIndex, taskFilter: taskFilter, processFields: processFields, ArvakConfig: ArvakConfig }
		});
	}
	...
	function populateMissingFieldValues(tasks) {
		...
		return TaskService.getProcessFields(processIds).then(function (processFields) {
			return TaskService.getProcessForms(processIds).then(function (response) {
				return UserService.getUser().then(function (userData) {
					return webWorkerTasks.createWebWorker({
						Webworker: Webworker,
						webWorkerFunction: webWorkerTasks.populateFieldValuesWorker,
						data: { tasks: tasks, processFields: processFields, formdefwithtasks: response.formdefwithtasks, userData: userData, ArvakConfig: ArvakConfig }
					});
				});
			});
		}).catch(function (error) {
  ...
  */
	function removeTask(task) {
		var userId = $rootScope.user.id.fieldValue;
		var appState = NavigationService.getAppState();
		var appDefId = appState.id;
  ...
  */
	function updateTasks(tasks) {
		var cleanTasks = _.map(tasks, function (task) {
			var instance = task.originalInstance;
			if (instance) {
				instance.taskToSubmit = task;
				instance.taskStatus = task.taskStatus;
				delete task.originalInstance;
				return instance;
			} else {
				if (task.taskdata) {
					instance = {
						id: task.taskdata.dataSourceInstanceId,
						taskToSubmit: task,
						taskStatus: task.taskStatus
					};
				} else {
					return _.omit(task, ['form', 'error']);
				}
			}
			return instance;
		});
		return formatUserTasksReduxModel(cleanTasks)
  ...
  */
	function loadAndPrepareAllTasks() {
		_dataLoading = true;
		sendDataLoadingFlag(_dataLoading);
		return getTasksFromServerPromiseBased()
		//.then(transformAllTasks)
		//.then(populateMissingFieldValues)
		.then(formatUserTasksReduxModel).then(function (userTasks) {
			...
			getUserTasks().then(function (userTasksObj) {
				...
				if (!_.isUndefined(userTasksObj.instances)) {
					_.each(userTasks.list, function (clientId) {
						...
						if (currentTask) {
							//TODO: check 'status' property from new contract
							if (currentTask.taskStatus === ArvakConfig.taskStatus.open) {
	...
	function getLatestAfterSubmit() {
		if (!latestAfterSubmitPromise) {
			latestAfterSubmitPromise = getLatestAfterSubmitPromise();
		}
		return latestAfterSubmitPromise;
	}

	function getTasksFromServerPromiseBased(userTaksSettings) {
		if (!tasksFromServerPromise) {
			tasksFromServerPromise = getTasksFromServer(userTaksSettings);
		}
		return tasksFromServerPromise;
	}
 ...
 * @returns {*}
 */
	function getLatestAfterSubmitPromise() {
		if (!ConnectionService.isOnline()) {
			_dataLoading = false;
			sendDataLoadingFlag(_dataLoading);
			return $q.when([]).finally(function () {
				latestAfterSubmitPromise = null;
			});
		}
		_dataLoading = true;
		sendDataLoadingFlag(_dataLoading);
		return $timeout(function () {}, 5000).then(getUserTaskSettings).then(getTasksFromServerPromiseBased)
		//.then(transformAllTasks)
		//.then(populateMissingFieldValues)
		.then(formatUserTasksReduxModel).then(function (userTasks) {
			...
			return UserService.getUserSettings().then(function (userSettings) {
				...
				var userOpenTasks = userSettings.general.openTasksIds;
				return getUserTasks().then(function (userTasksObj) {
					//Remove stale tasks
					userOpenTasks = _.map(userOpenTasks, function (taskId) {
			...
			console.log("destroying getTasksFromServer Promise");
			tasksFromServerPromise = null;
		});
	}
	...
	function flushTasks() {
		...
		UserService.getUserSettings().then(function (userSettings) {
			userSettings.general.tasksCount = {};
			userSettings.general.lastSyncTime = null;
			userSettings.general.removedTaskInstIds = [];
			UserService.setUserSettings(userSettings);
		});

		PouchService.flushDocs(PouchService.taskDB);
	...
	function setAutoPolling() {
		...
		pollingTasksInterval = $interval(function () {
			if (ConnectionService.isOnline() && !isAutoPollingRunning) {
				processUnsentTasks().then(getLatestAfterSubmit).catch(function (error) {
...
},{"../redux/actions":354,"./taskHelperService":352,"./webWorkerTasks":353}],352:[function(require,module,exports){
"use strict";
...
exports.getTaskFormById = getTaskFormById;
function getTasks(idArray, instancesByIdDictionary) {
	return _.map(idArray, function (id) {
		return instancesByIdDictionary[id];
	});
}

function filterTasks(filters, tasks) {
	var taskArray = [];
	...
	_.each(tasks, function (task, key) {
		...
		filters.every(function (filter) {
			passing = filterIterator(task, filter);
			return passing;
		});
...
function filterIterator(task, filter) {
	var passes = false;
	if (filter.formId && filter.formId === parseInt(task.formDefinitionId)) {
		passes = true;
	} else if (filter.formId) {
		passes = false;
		return passes;
	}
	if (filter.statusId && filter.statusId === parseInt(task.taskStatus)) {
		passes = true;
	} else if (filter.statusId) {
		passes = false;
		return passes;
	}
	if (filter.instances && !_.isEmpty(filter.instances)) {
		if (_.includes(filter.instances, task.id)) {
	...
	if (filter.fields && !_.isEmpty(filter.fields)) {

		var filterField = filter.fields[0]; // TODO:  NEED TO MAKE THIS WORK FOR MORE THSN ONE FIELD.
		//if(task.dsim.fieldDict[filterField.id]){  

		if (task.dsim.fieldDict[filterField.id] && filterField && !_.isUndefined(filterField.fieldValue)) {
			if (task.dsim.fieldDict[filterField.id].fieldValue !== filterField.fieldValue) {
				return false;
			}
		}
...
function createFilter(filterObj) {
	...
	return _.defaults(filterObj, defaultFilter);
}

function getTaskFormById(id, taskForms) {
	var taskForm = _.find(taskForms, function (form) {
		return form.formDefModel.id === id;
	});
	return taskForm;
}
...
exports.transFormTasksWorker = transFormTasksWorker;
exports.populateFieldValuesWorker = populateFieldValuesWorker;
exports.formatUserTasksReduxModelWorker = formatUserTasksReduxModelWorker;
exports.buildForNormalTasksWebWorker = buildForNormalTasksWebWorker;
exports.sortRawTasksWebWorker = sortRawTasksWebWorker;
function createWebWorker(_ref) {
	var Webworker = _ref.Webworker,
	    webWorkerFunction = _ref.webWorkerFunction,
	    data = _ref.data;

	var myWorker = Webworker.create(webWorkerFunction);
	var pathOrigin = window.location.origin;
	if (window.cordova) {
		pathOrigin = window.cordova.file.applicationDirectory + 'www';
	}
	var webWorkerData = Object.assign(data, { ArvakConfig: { ddTypes: data.ArvakConfig.ddTypes, taskStatus: data.ArvakConfig.taskStatus }, pathOrigin: pathOrigin });
	return myWorker.run(webWorkerData).then(function (result) {
		//debugger;
		return result;
	}).catch(function (error) {
		debugger;
		console.error('Error from ' + webWorkerFunction.name + ' function');
		throw error;
	});
}

function transFormTasksWorker(_ref2) {
	var tasks = _ref2.tasks,
	    ArvakConfig = _ref2.ArvakConfig,
	    pathOrigin = _ref2.pathOrigin;


	function getInstanceStatus(instance) {
		if (instance.taskdata) {
			return instance.taskdata.statusId;
		} else if (instance.statusId) {
	...
	function addSystemFields(instance) {
		//Add Task Status
		var status = getInstanceStatus(instance);
		var fieldValue = getStatusLabelById(status);
		...
		if (statusSourceFields) {
			var instField = _.find(instance.instFields, function (field) {
				return _.find(statusSourceFields, function (statusSrcFld) {
					return statusSrcFld.sourceField.dsTypeId === instance.dsTypeId && statusSrcFld.sourceField.sourceFieldId === field.sourceFieldId;
				});
			});
			if (instField) {
				fieldValue = getStatusLabelById(instField.fieldValue);
	...
	function transformTask(task) {
		...
		_.each(clonedTask.dsim.instances, function (instance) {
			var referredInstKeyData = null;
			_.each(instance.fields, function (field) {

				if (field.ddTypeId == ArvakConfig.ddTypes.relationship || field.ddTypeId == ArvakConfig.ddTypes.relationship || field.ddTypeId == ArvakConfig.ddTypes.relationship_org_parent) {

					referredInstKeyData = _.filter(instance.fields, function (field) {
						return field.isKey === true;
					});
					if (!_.isEmpty(referredInstKeyData)) {
						referredInstKeyData.sort(function (a, b) {
							return a.sortOrder - b.sortOrder;
						});
						field.referredInstKeyData = referredInstKeyData;
					}
				}
	...
	importScripts(pathOrigin + '/lib/lodash/lodash.js');
...
function populateFieldValuesWorker(_ref3) {
	var tasks = _ref3.tasks,
	    processFields = _ref3.processFields,
	    formdefwithtasks = _ref3.formdefwithtasks,
	    userData = _ref3.userData,
	    ArvakConfig = _ref3.ArvakConfig,
	    pathOrigin = _ref3.pathOrigin;

	importScripts(pathOrigin + '/lib/lodash/lodash.js');
	importScripts(pathOrigin + '/lib/moment/moment.js');
   ...
   var formDef = _.find(formdefwithtasks, function(formDef){
   ...
   */
			...
			_.each(task.fieldsDict, function (taskField) {
				...
				formatFieldValue({ taskField: taskField, processField: processField, task: task, userData: userData, ArvakConfig: ArvakConfig });
			});
		});
	...
	function formatFieldValue(_ref4) {
		var taskField = _ref4.taskField,
		    processField = _ref4.processField,
		    task = _ref4.task,
		    userData = _ref4.userData,
		    ArvakConfig = _ref4.ArvakConfig;
...
function formatUserTasksReduxModelWorker(_ref5) {
	var tasks = _ref5.tasks,
	    ArvakConfig = _ref5.ArvakConfig,
	    pathOrigin = _ref5.pathOrigin;
	...
	_.each(tasks, function (task) {
		var taskId = task.id;
		if (_.isNil(task.taskStatus)) {
			task.taskStatus = ArvakConfig.taskStatus.open;
		}
...
function buildForNormalTasksWebWorker(_ref6) {
	var tasks = _ref6.tasks,
	    taskFilter = _ref6.taskFilter,
	    processFields = _ref6.processFields,
	    fieldSettings = _ref6.fieldSettings,
	    nextViewId = _ref6.nextViewId,
	    ArvakConfig = _ref6.ArvakConfig,
	    pathOrigin = _ref6.pathOrigin;
	...
	function init() {
		...
		_.each(tasks, function (task) {
			...
			var row = {
				id: nextViewId, //NavigationService.getCurrentView().to,
				task: task,
				fields: []
			};

			taskFields = task.fields;
			_.each(taskFilter, function (chosenFieldId, fieldIndex) {

				var chosenField = getFieldBySrcAndDSType(chosenFieldId, processFields);
				var keyFieldDict = chosenField.sourceField.dsTypeId + ':' + chosenField.sourceField.dsId + ':' + chosenField.sourceField.sourceFieldId + ':0';
				var matchedField = null;
				if (task.fieldsDict[keyFieldDict]) {
					matchedField = task.fieldsDict[keyFieldDict];
				}

				var dataRowMatchedField = buildRowField({
					matchedField: matchedField,
					chosenFieldId: chosenFieldId,
				...
				var matchedCopy = dataRowMatchedField.rowField;
				row.fields.push(matchedCopy);
			});
	...
	function getFieldBySrcAndDSType(id, fields) {
		if (_.isArray(fields)) {
	...
	function buildRowField(params) {
		var matchedField = params.matchedField;
		var chosenFieldId = params.chosenFieldId || '';
		var chosenField = params.chosenField;
		var fieldIndex = params.fieldIndex;
		var instance = params.instance || {};
		var fieldSettings = params.fieldSettings || {};

		var rowField;

		if (!_.isNil(matchedField)) {

			if (_.isUndefined(matchedField.fieldValue)) {
				//empty value
				rowField = { id: -1, name: "-", fieldValue: "-" };
			} else {
				//console.log("Checking for optionLabel");
				if (fieldSettings[chosenFieldId.replace(":", "")]) {
					var matchedFieldCopy = _.cloneDeep(matchedField);

					if (matchedFieldCopy.optionLabel) {
						matchedFieldCopy.fieldValue = matchedFieldCopy.optionLabel;
					} else if (matchedFieldCopy.fieldLabel) {
						matchedFieldCopy.fieldValue = matchedFieldCopy.fieldLabel;
					}

					var statusSourceFieldValue = getStatusSourceFieldValue(instance, matchedField, chosenField);
					if (statusSourceFieldValue) {
						matchedFieldCopy.fieldValue = statusSourceFieldValue;
					} else if (chosenField && (chosenField.ddTypeId === ArvakConfig.ddTypes.completed_on || chosenField.widgetTemplate === "datetime")) {

						rowField = _.cloneDeep(matchedField);

						var date = moment(parseInt(matchedField.fieldValue));
						if (date.isValid()) {
							matchedFieldCopy.fieldValue = date.format("MM/DD/YYYY HH:mm");
						} else {
							matchedFieldCopy.fieldValue = '-';
						}
					} else {
						matchedFieldCopy.fieldValue = getSubstring(matchedFieldCopy.fieldValue, fieldSettings[chosenFieldId.replace(":", "")].charDisplay, fieldSettings[chosenFieldId.replace(":", "")].charNumber);
					}
					rowField = matchedFieldCopy;
				} else {

					var matchedFieldCopy = _.cloneDeep(matchedField);
					if (matchedFieldCopy.fieldLabel) {
						matchedFieldCopy.fieldValue = matchedFieldCopy.fieldLabel;
					}
					rowField = matchedFieldCopy;
				}
		...
		if (chosenField && !_.isEmpty(chosenField.compositeDefinition)) {
			rowField = {
				id: chosenField.id,
				name: chosenField.name,
				fieldValue: _.reduce(chosenField.compositeDefinition, function (accum, compositeField) {
					var keyField = chosenField.sourceField.dsTypeId + ':' + chosenField.sourceField.dsId + ':' + compositeField.sourceFieldId + ':0';
					var compDefField = instance.fieldsDict[keyField] || {};
					var fieldValue = compDefField.optionLabel || compDefField.fieldString || compDefField.fieldValue || '';
					return _.isEmpty(fieldValue) ? accum : _.isEmpty(accum) ? fieldValue : accum + ' ' + fieldValue;
				}, '')
			};
		}
		...
		if (fieldSettings[chosenFieldId.replace(":", "")] && fieldSettings[chosenFieldId.replace(":", "")].isSearchable) {
			if (fieldSettings[chosenFieldId.replace(":", "")].isSearchable === true) rowField.isSearchable = true;
		} else {
			rowField.isSearchable = false;
		}

		if (_.has(chosenField, 'sourceField')) {
			rowField.dsId = rowField.dsId || chosenField.sourceField.dsId;
			rowField.dsTypeId = rowField.dsTypeId || chosenField.sourceField.dsTypeId;
			rowField.sourceFieldId = rowField.sourceFieldId || chosenField.sourceField.sourceFieldId;
		}
		rowField.fieldValue = rowField.fieldValue ? rowField.fieldValue : '-';
	...
	function getStatusSourceFieldValue(instance, matchedField, chosenField) {
		var fieldValue = null;
		var statusSourceFields = instance.statusSourceFields;
		if (statusSourceFields) {
			var statusSrcField = _.find(statusSourceFields, function (statusSrcField) {
				return statusSrcField.sourceField.sourceFieldId === chosenField.sourceField.sourceFieldId && statusSrcField.sourceField.dsTypeId === chosenField.sourceField.dsTypeId;
			});

			if (statusSrcField) {
				var optionEntry = _.find(statusSrcField.ddEntry.options.optionsList, function (optionEntry) {
					return optionEntry.uuid === matchedField.fieldValue;
				});

				if (optionEntry) {
					fieldValue = optionEntry.label;
				}
			}
		}
	...
	function getSubstring(value, direction, numChars) {
		if (_.isString(value)) {
			value = value.replace(/(\r\n|\n|\r)/gm, '');
		}
		if (!direction) {
			direction = { value: 'allChars' };
		}
		if (_.isString(value) && direction.value !== "allChars") {
			if (direction.value === "right") {
				numChars.value = -Math.abs(parseInt(numChars.value));
				return value.substr(parseInt(numChars.value));
			} else if (direction.value === "left") {
				return value.substr(0, parseInt(numChars.value));
			}
		}
		return value;
	}
...
function sortRawTasksWebWorker(_ref7) {
	var tasks = _ref7.tasks,
	    sortDirection = _ref7.sortDirection,
	    fieldIndex = _ref7.fieldIndex,
	    taskFilter = _ref7.taskFilter,
	    processFields = _ref7.processFields,
	    pathOrigin = _ref7.pathOrigin;
	...
	function init() {
		fieldIndex = fieldIndex >= taskFilter.length ? taskFilter.length - 1 : fieldIndex;
		var chosenFieldId = taskFilter[fieldIndex];
		var chosenField = getFieldBySrcAndDSType(chosenFieldId, processFields);
		var keyFieldDict = null;
		if (chosenField) {
			keyFieldDict = chosenField.sourceField.dsTypeId + ':' + chosenField.sourceField.dsId + ':' + chosenField.sourceField.sourceFieldId + ':0';
		}
		var normalizedTasks = _.map(tasks, function (task) {
			var hasFieldDict = keyFieldDict && _.has(task, 'fieldsDict');
			var field = hasFieldDict ? task.fieldsDict[keyFieldDict] : null;
			var value = '-';
			if (field) {
				value = field.optionLabel || field.fieldString || value;
			}
			return [value, task];
		});

		normalizedTasks = normalizedTasks.sort(cmp);
		if (sortDirection === 'desc') {
			normalizedTasks.reverse();
		}
		return denormalizeSortArr(normalizedTasks);
	}
	...
	function denormalizeSortArr(normalized) {
		var denormalized = [];
		_.each(normalized, function (normalArray) {
			denormalized.push(normalArray[1]);
		});
		return denormalized;
	}
...
var LOAD_VIEWS = exports.LOAD_VIEWS = 'LOAD_VIEWS';
var UPSERT_TASKS = exports.UPSERT_TASKS = 'UPSERT_TASKS';
var REMOVE_TASK = exports.REMOVE_TASK = 'REMOVE_TASK';
var BUILD_MENU = exports.BUILD_MENU = 'BUILD_MENU';
var BUILD_VIEW = exports.BUILD_VIEW = 'BUILD_VIEW';
var CHANGE_STATUS = exports.CHANGE_STATUS = 'CHANGE_STATUS';
var FLUSH_TASKS = exports.FLUSH_TASKS = 'FLUSH_TASKS';
var REMOVE_VIEWS = exports.REMOVE_VIEWS = 'REMOVE_VIEWS';

var upsertTasks = exports.upsertTasks = function upsertTasks(userTasks, userId, appDefId) {
	return {
		type: UPSERT_TASKS,
		userTasks: userTasks,
		userId: userId,
...
var changeStatus = exports.changeStatus = function changeStatus(taskId, newStatusId, userId, appDefId) {
	return {
		type: CHANGE_STATUS,
		taskId: taskId,
...
var loadTasks = exports.loadTasks = function loadTasks(userTasks, userId, appDefId) {
	return {
		type: LOAD_TASKS,
		userTasks: userTasks,
		userId: userId,
...
var loadViews = exports.loadViews = function loadViews(views, userId, appDefId) {
	return {
		type: LOAD_VIEWS,
		views: views,
		userId: userId,
...
var buildMenu = exports.buildMenu = function buildMenu(menuItems) {
	return {
		type: BUILD_MENU,
		menuItems: menuItems
	};
};
var buildView = exports.buildView = function buildView(params) {
	return {
		type: BUILD_VIEW,
		viewId: params.id,
...
var removeTask = exports.removeTask = function removeTask(task, userId, appDefId) {
	return {
		type: REMOVE_TASK,
		task: task,
		userId: userId,
...
var flushTasks = exports.flushTasks = function flushTasks(userId, appDefId) {
	return {
		type: FLUSH_TASKS,
		userId: userId,
		appDefId: appDefId
...
var removeViews = exports.removeViews = function removeViews(userId, appDefId) {
	return {
		type: REMOVE_VIEWS,
		userId: userId,
		appDefId: appDefId
...
var rootReducer = (0, _redux.combineReducers)({
  tasks: _tasks.tasks, views: _views.views
}); /*
     * Pure functions format:
...
},{"./tasks":356,"./views":357,"redux":345}],356:[function(require,module,exports){
'use strict';
...
function createUserTasks() {
	var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { list: [], instances: {} };
	var action = arguments[1];
	...
	switch (action.type) {
		...
		case _actions.UPSERT_TASKS:

			return {
				list: _.union(state.list, action.userTasks.list),
				instances: Object.assign({}, state.instances, action.userTasks.instances)
			};
...
function removeTask() {
	...
	switch (action.type) {
		...
		case _actions.REMOVE_TASK:
			var newInstances = state.instances;
			delete newInstances[action.task.id];
			return {
				list: _.reject(state.list, function (taskId) {
					return taskId === action.task.id;
				}),
...
//Reducer for handles main  task list.
function tasks() {
	var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
	var action = arguments[1];
	...
	var stateKey = action.userId + ':' + action.appDefId;
	switch (action.type) {

		case _actions.CHANGE_STATUS:
			var userTasks = state[stateKey];
			var taskId = action.taskId;
			return Object.assign({}, state, _defineProperty({}, stateKey, {
				list: [].concat(_toConsumableArray(userTasks.list)),
				instances: Object.assign({}, userTasks.instances, _defineProperty({}, taskId, changeTaskStatus(userTasks.instances[taskId], action)))
			}));

		case _actions.LOAD_TASKS:
			return Object.assign({}, state, _defineProperty({}, stateKey, action.userTasks));

		case _actions.UPSERT_TASKS:
			return Object.assign({}, state, _defineProperty({}, stateKey, createUserTasks(state[stateKey], action)));

		case _actions.REMOVE_TASK:
			return Object.assign({}, state, _defineProperty({}, stateKey, removeTask(state[stateKey], action)));

		case _actions.FLUSH_TASKS:
			return Object.assign({}, state, _defineProperty({}, stateKey, { list: [], instances: {} }));
...
},{"../actions":354}],357:[function(require,module,exports){
'use strict';
...
var _taskHelperService = require('../../bound-actions/taskHelperService');
...
function createTaskView() {
	var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { list: [], filters: [] };
	var action = arguments[1];
	...
	switch (action.type) {
		...
		case _actions.BUILD_VIEW:
			var cleanParentFilters = _.filter(state.filters, function (filter) {
				var updatedFilter = _.find(action.filters, function (actionFilter) {
					return actionFilter.id === filter.id;
				});
			...
			var filteredTasks = taskHelper.filterTasks([].concat(_toConsumableArray(cleanParentFilters), _toConsumableArray(action.filters)), action.tasks);
			return {
				list: filteredTasks,
				filters: [].concat(_toConsumableArray(cleanParentFilters), _toConsumableArray(action.filters))
			};

		case _actions.LOAD_TASKS:
			return {
				list: taskHelper.filterTasks(state.filters, action.userTasks.instances),
				filters: state.filters
			};

		case _actions.UPSERT_TASKS:
			/**
    * We remove the new tasks.list from the view.list with _.difference because maybe
    * they're no longer belong to the current view, otherwise they will added in _.union
    */
			var staleList = _.map(state.list, function (taskId) {
				return parseInt(taskId);
			});
			var newList = _.difference(staleList, action.userTasks.list); //Array that contains all the elements that are in A but not in B
			var filteredTasks = taskHelper.filterTasks(state.filters, action.userTasks.instances);
			return {
				list: _.union(newList, _.map(filteredTasks, function (taskId) {
					return parseInt(taskId);
				})),
				filters: state.filters
			};
		...
		case _actions.REMOVE_TASK:
			return {
				list: _.filter(state.list, function (taskId) {
					return taskId !== action.task.id;
				}),
				filters: state.filters
			};
...
function createUserViews() {
	...
	switch (action.type) {
		...
		case _actions.BUILD_VIEW:
			return Object.assign({}, state, _defineProperty({}, action.viewId, createTaskView(state[action.viewId], action)));

		case _actions.REMOVE_TASK:
		case _actions.UPSERT_TASKS:
		case _actions.LOAD_TASKS:
		case _actions.FLUSH_TASKS:
			return _.mapValues(state, function (view) {
				return createTaskView(view, action);
			});

		case _actions.REMOVE_VIEWS:
			return {};
...
//Reducer for handles main view list.
function views() {
	var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
	var action = arguments[1];
	...
	switch (action.type) {
		...
		case _actions.LOAD_VIEWS:
			return Object.assign({}, state, _defineProperty({}, stateKey, action.views));
		...
		case _actions.LOAD_TASKS:
		case _actions.FLUSH_TASKS:
			return Object.assign({}, state, _defineProperty({}, stateKey, createUserViews(state[stateKey], action)));
...
},{"../../bound-actions/taskHelperService":352,"../actions":354}]},{},[350])
